...
def percent_escape(str):
	return re.sub('[\x80-\xff /&]', lambda x: '%%%02X' % unpack('B', x.group(0).encode('latin-1'))[0], str)
...
newFilePat = re.compile(r'.*\((\./.*\.tex)')
warnPat = re.compile(r'LaTeX Warning.*?input line (\d+).$')
errPat = re.compile(r'^([./\w\x7f-\xff]+\.tex):(\d+):.*')
incPat = re.compile(r'.*\<use (.*?)\>')
miscWarnPat = re.compile(r'LaTeX Warning:.*')
